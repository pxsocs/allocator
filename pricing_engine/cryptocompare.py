...
from connections.connections import url_request
import pandas as pd
...
def historical(ticker, fx='USD', parsed=True):
    ...
    response = requests.get(globalURL)
    if response.json()['Message'] == 'You are over your rate limit please upgrade your account!':
        return("API_LIMIT")
